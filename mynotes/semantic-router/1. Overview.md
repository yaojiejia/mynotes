...
Deep dives, all read from source at commit `63e91c2`: [[2. Signal Extraction]] · [[3. Decision Engine]] · [[4. Plugin Chain]]
...
> **Correction from source.** The code has **21** configurable signal types, not 13. The eight the paper omits are `reask`, `structure`, `kb`, `conversation`, `event`, `metadata`, `classifier` and `input_modality`. See [[2. Signal Extraction]].
...
Each layer of this walkthrough is expanded on its own page: [[2. Signal Extraction]], [[3. Decision Engine]], [[4. Plugin Chain]].
...
> **Correction from source.** The catalog holds **16** algorithms and tiers them. Eight are `experimental`, including every classical ML method below and AutoMix. Thompson Sampling and GMTRouter do not appear in the code at all. See [[4. Plugin Chain]].
